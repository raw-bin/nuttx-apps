...
int main(int argc, FAR char *argv[])
{
  ...
  for (int i = 0; i < count; i++)
  {
    ...
    if (rc != *rd)
    {
      printf("Unexpected compile error %d (expected %d)\n", rc, *rd);
      printf("Pattern is: %s\n", pattern);
      return 1;
    }
    ...
    for (; *subjects != NULL; subjects++)
    {
      ...
      {
        printf("\n");
        printf(stderr, "Unexpected match error %d (expected %d)\n", rc, *rd);
        printf(stderr, "Pattern is: %s\n", pattern);
        printf(stderr, "Subject is: %s\n", *subjects);
        return 1;
      }
      ...
      if (rc == 0)
      {
        for (int j = 0; j < CAPCOUNT; j++)
        {
          ...
          {
            printf("\n");
            printf(stderr, "Mismatched results for successful match\n");
            printf(stderr, "Pattern is: %s\n", pattern);
            printf(stderr, "Subject is: %s\n", *subjects);
            printf(stderr, "Result %d: expected %d %d received %d %d\n",
